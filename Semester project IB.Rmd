...
PkgNames <- c("dplyr", "magrittr", "ggplot2", "reshape2", "purrr","utf8", "data.table", "plyr", "rcompanion", "corrplot", "car", "multcomp", "DescTools", "vegan", "randomForest", "caTools")
...
rf3
prediction = predict(rf3, newdata = test[-1])
cm <-  table(test[,1], prediction)
cm
...
```{r}
Cells <- subset(Cells_cl, Cells_cl$Sample == 'control' | Cells_cl$Sample == 'Trombin')
rf_cell<- randomForest(x = Cells[,2:17],y = Cells$Sample, ntree = 1000,do.trace = 100) 
